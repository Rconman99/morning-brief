# RESUME BUILD — Pick Up Where We Left Off

Read CLAUDE.md first. Follow all rules (venv paths, envelope schema, import strategy, no questions, no stopping).

## WHAT'S ALREADY DONE (DO NOT REBUILD)

The previous session completed these builds and committed them:

### v2.0 (12 core modules) ✅
### v3.0 (opportunity scanner, risk dashboard) ✅
### v4.0 (insider_tracker, position_sizer, trade_memory, scenario valuations, VWAP/volume profile) ✅
### Interactive Learning System ✅
- `modules/brief_learning.py` — 1,264 lines, 46 concepts, ELI5+Detailed, quizzes, guided tour, learning path, how-to-use guide
- `modules/brief_html.py` — wired with learning imports (`ll`, `st`, panel, tour, etc.)
- Committed: `feat: add interactive learning system to HTML brief`

### E*TRADE Portfolio Sync ✅
- Committed: `feat: add E*TRADE portfolio sync with three-tier fallback`

### Pipeline has been run successfully ✅
- Morning briefs exist for 2026-02-26 and 2026-02-27
- Pipeline run log committed

## WHAT STILL NEEDS TO BE DONE

### 1. Missing test: `tests/test_brief_learning.py`

The learning system module has no test file. Create one:
...
### 2. Missing test: `tests/test_brief_html.py`

The HTML dashboard module has no test file. Create one that:
- Uses the mock_envelopes fixture from conftest.py
- Verifies generate_html() returns valid HTML
- Verifies learning system elements are present (data-concept attributes, learn-panel div, LEARN_CONCEPTS script tag)
- Verifies all 16 sections render (grep for section-title class)
- Verifies verdict cards render for mock data
...
### 4. Run the full pipeline to generate a fresh brief

```bash
.venv/bin/python scripts/run_all.py
```

Verify the HTML brief opens and the learning system works (panel HTML present, JS injected, concepts loaded).

### 5. Commit

```
feat: add learning system tests + brief_html tests
```

---

## AFTER TESTS PASS — BRUTAL ANALYSIS V2 VERIFICATION

The learning system was built incorporating the brutal analysis fixes. Verify these are present in `modules/brief_learning.py`:

1. **46 concepts** (not 30) — should include market basics, psychology, all technicals
2. **ELI5 + Detailed toggle** — every concept has both `eli5` and `detailed` fields
3. **4-tier learning path** — Foundations → Core → Advanced → Expert
4. **Quizzes** — every concept has at least 1 quiz question
5. **Psychology concepts** — FOMO, loss aversion, confirmation bias, overtrading, first loss, patience
6. **Market fundamentals** — what is a stock, market hours, order types, brokers, bull/bear, what moves prices
7. **"How to Use This Brief" guide** — `build_how_to_use_html()` function exists
8. **Guided tour** — `build_tour_html()` function exists
9. **Connected narratives** — `connects_narrative` field (not just `connects_to` links)

If ANY of these are missing, add them. If all present, you're done.

---

*This is a tool specification, not financial advice. All trading decisions are yours.*
