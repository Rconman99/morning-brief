# Morning Brief — 2026-06-10

## Module Status

- ✅ **Journal**: success
- ⚠️ **Earnings**: partial
- ✅ **Valuation**: success
- ❌ **Portfolio**: error
- ❌ **Technical**: error
- ⚠️ **Sentiment**: partial
- ⚠️ **Options**: partial
- ⚠️ **Scanner**: partial
- ✅ **Risk**: success
- ⚠️ **Scorecard**: partial
- ✅ **Memory**: success
- ✅ **Insider**: success
- ✅ **Sizer**: success
- ⚠️ **Congress**: partial
- ⚠️ **Polymarket**: partial
- ✅ **EconCalendar**: success
- ❌ **Macro**: error
- ❌ **SectorRotation**: error
- ⚠️ **UnusualOpts**: partial
- ⚠️ **SocialSent**: partial
- ✅ **Dividends**: success
- ❌ **CorrHeatmap**: error
- ✅ **Backtester**: success
- ✅ **Alerts**: success
- ⬜ **Last30Days**: missing
- ❌ **SocialIntel**: error

## Risk Dashboard — LOW_RISK
Favorable conditions — lean into high-conviction setups

### LONG (This Month+)
- 🟡 SAMPLE_AAPL earnings tone -1.8 — management hedging

## Economic Calendar — MODERATE
*CPI Release 2026-06-13*

### This Week
- 🟡 **CPI Release** (2026-06-13) — Consumer Price Index - inflation data
- 🟡 **PPI Release** (2026-06-14) — Producer Price Index - inflation at producer level

*Next FOMC: 2026-06-17 (7 days)*

## Macro Dashboard
*Data unavailable*

## Sector Rotation
*Data unavailable*

## Trade Journal Summary
- **Total Trades**: 30
- **Win Rate**: 63.3%
- **Patterns Detected**:
  - TSLA: 4 consecutive losses within 48hrs
  - Tuesday win rate 0% (8 trades)
- **Checklist Rules**:
  - RULE: After 2 consecutive losses on TSLA, stop trading that ticker for 24hrs
  - RULE: Avoid trading on Tuesdays (historically poor performance)

## Earnings Tone Analysis
### SAMPLE_AAPL
- **Tone Score**: -1.8 (scale: -5 to +5)
- **Confidence**: 0.32
- **Hedge/Definitive**: 15 / 7
- **Risk Factors**:
  - However, we expect some pressure on hardware margins in the near term due to increased component costs for our custom silicon
  - We anticipate headwinds from semiconductor supply constraints, particularly for advanced packaging at three-nanometer nodes
  - We might face additional challenges from local competitors gaining share in the mid-range smartphone segment
  - The uncertainty around AI monetization timelines is something the entire industry faces
  - There are certainly risk factors we are monitoring
- SAMPLE_AAPL management struck a mixed tone with 7 definitive and 15 hedging phrases. Identified 8 risk-related statements in the call.


## Valuation Comparisons
### NVDA vs AMD
- **Cheaper**: inconclusive
- Insufficient comparable metrics (0) to determine relative value between NVDA and AMD.

### AAPL vs MSFT
- **Cheaper**: inconclusive
- Insufficient comparable metrics (0) to determine relative value between AAPL and MSFT.

### GOOGL vs META
- **Cheaper**: inconclusive
- Insufficient comparable metrics (0) to determine relative value between GOOGL and META.


## Portfolio Overview
*Data unavailable*

## Options Analysis
### NVDA — *Skipped: No price data*

### AAPL — *Skipped: No price data*

### VOO — *Skipped: No price data*


## Verdict Scorecard
- **Verdicts Evaluated**: 0
- **Total Scored**: 0
- *No windows have elapsed yet — check back in 5 days*

## Watchlist Verdicts
- ⚪ **AAPL**: HOLD — Default — no strong signal in either direction
- ⚪ **AMD**: HOLD — Default — no strong signal in either direction
- ⚪ **GOOGL**: HOLD — Default — no strong signal in either direction
- ⚪ **META**: HOLD — Default — no strong signal in either direction
- ⚪ **MSFT**: HOLD — Default — no strong signal in either direction
- ⚪ **NVDA**: HOLD — Default — no strong signal in either direction
- ⚪ **VOO**: HOLD — Default — no strong signal in either direction

## Opportunity Scanner
*Scanned 22 tickers outside your watchlist*

## Trade Memory — Pattern Matching
*No matching patterns found in trade history*

## Insider Activity (SEC Form 4)
*No recent insider activity detected*

## Congressional Trading (STOCK Act)
*Signal: CLUSTER_DETECTED — 1 cluster signal(s) detected, 1 in your watchlist*
- **Trades**: 12 (8 buys, 4 sells)
- **Watchlist Matches**: 12

### Cluster Signals
- 🟢 **NVDA**: 3 politicians buying (~$3,450,000) ⭐ WATCHLIST
  - Politicians: Mark Green, Markwayne Mullin, Nancy Pelosi

### Watchlist Trades
- 🔴 **Katie Porter** (D) — Sale (Full) AAPL — $50,001 - $100,000 — 2026-02-25
- 🟢 **Josh Gottheimer** (D) — Purchase META — $15,001 - $50,000 — 2026-02-24
- 🟢 **Mark Green** (R) — Purchase NVDA — $50,001 - $100,000 — 2026-02-22
- 🔴 **Tommy Tuberville** (R) — Sale (Full) TSLA — $250,001 - $500,000 — 2026-02-20
- 🟢 **Tommy Tuberville** (R) — Purchase AMD — $100,001 - $250,000 — 2026-02-20
- 🟢 **Markwayne Mullin** (R) — Purchase NVDA — $250,001 - $500,000 — 2026-02-19
- 🟢 **Dan Crenshaw** (R) — Purchase MSFT — $100,001 - $250,000 — 2026-02-18
- 🟢 **Nancy Pelosi** (D) — Purchase NVDA — $1,000,001 - $5,000,000 — 2026-02-15
- 🔴 **Ro Khanna** (D) — Sale (Partial) GOOGL — $100,001 - $250,000 — 2026-02-14
- 🔴 **Michael McCaul** (R) — Sale (Partial) MSFT — $500,001 - $1,000,000 — 2026-02-12

*Source: sample_data | STOCK Act requires disclosure within 45 days*

## Technical Signals
*Data unavailable*

## News Sentiment

## Polymarket Prediction Markets
*12 markets scanned — Signal: VOLATILE*

### Big Movers (24h)
- 📈 **NYC temperature high above 50°F on March 5?** — +5.0% (Vol: $45,000)
- 📉 **Will the S&P 500 reach a new all-time high in March?** — -4.0% (Vol: $890,000)
- 📈 **Will Bitcoin drop below $70,000 in 2026?** — +4.0% (Vol: $1,200,000)
- 📈 **Will Iran close the Strait of Hormuz by March 31?** — +3.0% (Vol: $2,720,000)
- 📉 **Will it rain in London on March 5?** — -3.0% (Vol: $28,000)

*Categories: weather: 3 | crypto: 2 | economics: 2 | geopolitics: 1 | markets: 1 | politics: 1*

### Economics Signals (Macro Cross-Reference)
- 📊 **Will Fed decrease rates 50+ bps after March 2026?** — YES 0%
  - FOMC in 7 day(s) — 2026-06-17
- 📊 **Will the US enter a recession in 2026?** — YES 18%
  - FOMC in 7 day(s) — 2026-06-17

*Source: sample_data | Prediction markets are not financial advice — 80% of participants lose money*

## Unusual Options Activity
*No unusual activity detected*

## Social Sentiment
*Market mood: neutral | Most discussed: None*

*No trending tickers*


## Strategy Backtest
*Simulated P&L: $+0.00 | Win rate: 0.0% | Alpha: +0.000 | Sharpe: 0.00*


---
*This is analysis, not financial advice. All trading decisions are yours.*
